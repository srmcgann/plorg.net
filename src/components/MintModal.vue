      ...
      <div v-else-if="stage==1" style="margin-top:-15px;">
        ...
        <div v-if="state.uploadedFileType == 'application/x-zip-compressed' || state.uploadedFileType == 'application/zip'">
          ...
          <button
            ...
            style="background: #289;left: 0;color: #fff;text-shadow: 2px 2px 2px #000;padding-left: 20px;padding-right: 20px;position: relative; left: 50%;font-size: 24px;transform: translate(-50%);border-radius: 30px; margin-left: auto; margin-right: auto;margin-bottom:20px;"
        ...
        <div
          v-else-if="state.uploadedFileType.indexOf('image/') !== -1"
          class="imageThumb"
          :class="{'portrait': assetLoaded && !(thumbnail.width<asset.height), 'landscape': assetLoaded && (thumbnail.width<asset.height),'showSpinner':shotInProgress}"
          :style="'background-image: url('+state.uploadedFileThumbnail+')'"
        ...
        <button
          ...
          @click="next()"
          ref="backButton"
          style="background: #289;left: 0;color: #fff;text-shadow: 2px 2px 2px #000;padding-left: 20px;padding-right: 20px;position: relative; left: 50%;font-size: 24px;transform: translate(-50%);border-radius: 30px;margin-left: auto; margin-right: auto;"
        >accept thumbnail</button>
        <button
          class="cancelButton"
...
export default{
  ...
  methods:{
    ...
    mint(){
      ...
      .then(res => res.json()).then(data => {
        if(data[0]){
          window.location.href = window.location.origin + '/t/' + data[2]
        }
      })
    }
  },
